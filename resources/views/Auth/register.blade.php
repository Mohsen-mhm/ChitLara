        ...
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
            ...
            <div
                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" action="{{ route('register.confirm') }}" method="POST">
                        @csrf
                        <div class="grid gap-6 mb-6 md:grid-cols-2">
                            <div>
                                <div class="flex justify-start items-start">
                                    <label for="name"
                                           class="transition block mb-2 text-sm font-medium text-gray-900 dark:text-white @error('name') text-red-600 dark:text-red-500 @enderror">{{ __('auth.name') }}
                                        &nbsp;</label>
                                ...
                                @error('name')
                                <span
                                    class="mt-2 text-sm text-red-600 dark:text-red-500 font-medium">{{ $message }}</span>
                                @enderror
                            </div>
                            <div>
                                <div class="flex justify-start items-start">
                                    <label for="username"
                                           class="transition block mb-2 text-sm font-medium text-gray-900 dark:text-white @error('username') text-red-600 dark:text-red-500 @enderror">{{ __('auth.username') }}
                                        &nbsp;</label>
                                    <svg class="w-2 h-2 text-red-800 dark:text-red-600" aria-hidden="true"
                                         xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                         viewBox="0 0 24 24">
                        ...
                        <div>
                            <div class="flex justify-start items-start">
                                <label for="password_confirmation"
                                       class="transition block mb-2 text-sm font-medium text-gray-900 dark:text-white @error('password_confirmation') text-red-600 dark:text-red-500 @enderror">{{ __('auth.password_confirmation') }}
                                    &nbsp;</label>
                                <svg class="w-2 h-2 text-red-800 dark:text-red-600" aria-hidden="true"
                                     xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                     viewBox="0 0 24 24">
                            ...
                            <input type="password" name="password_confirmation" id="password_confirmation"
                                   placeholder="••••••••"
                                   class="transition bg-gray-50 border border-gray-400 text-gray-900 sm:text-sm rounded-lg focus:ring-purple-600 focus:border-purple-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 @error('password_confirmation') border-red-500 dark:border-red-500 @enderror"
                                   required>
                        ...
                        <p class="text-sm font-light text-gray-600 dark:text-gray-400 transition">
                            {{ __('auth.already_signup') }} <a href="{{ route('login') }}"
                                                               class="font-medium transition text-purple-600 hover:underline dark:text-purple-500">{{ __('auth.sign_in') }}</a>
                        </p>
                    </form>
                </div>
            </div>
