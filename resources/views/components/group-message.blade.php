@if($message->attachment)
    <div class="flex items-start my-2 px-1">
        <div
...
@else
    <div class="flex items-end my-2 px-1">
        {!! \App\Helper\Helper::generateAvatar($message->user->username, $message->user->avatar) !!}
        <div
            class="flex flex-col relative w-5/6 max-w-[400px] leading-1.5 transition p-4 ml-2 border-gray-200 bg-indigo-700 hover:bg-indigo-800 dark:hover:bg-indigo-800 rounded-xl rounded-es-none">
            <div class="flex items-center space-x-2 rtl:space-x-reverse">
            <span
                class="text-sm font-semibold text-yellow-300 dark:text-yellow-300">{{ $message->user->name }}</span>
            </div>
            @if($message->message)
                <p dir="ltr" class="text-base font-mono font-bold py-2 text-white dark:text-gray-100">
                    {!! nl2br(str_replace(["\r\n", "\r", "\n"], '<br>', urldecode($message->message))) !!}
                </p>
            @endif
